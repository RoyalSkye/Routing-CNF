...
class TSPTrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            val_list = self._val_and_stat(dir, paths, val_episodes=1000)
            self.result_log.append('val_score', epoch, val_list)
        ...
        """
            ...
            Phase 2 mode == "adv":
                One pretrain model -> several experts
                    ...
                    - Train on the best model w.r.t. adv ins.
        """
            ...
            if mode == "nat":
                ...
                for i in range(self.num_expert):
                    ...
                    for j in range(self.num_expert):
                        ...
                        losses.append(loss)
                    self._update_model(scores, losses, type="instance_choice")
            else:
    ...
    def _update_model(self, scores, losses, type="instance_choice"):
        """
            Routing Mechanism:
                a. Each instance chooses its best expert
                b. Each expert chooses TopK instances
        """
        batch_size = scores.size(0)
        if type == "instance_choice":
            _, id = scores.min(1)  # (batch) - instance choice
        elif type == "expert_choice":
            _, id = scores.topk(batch_size // self.num_expert, dim=0, largest=False)  # (k, num_expert) - expert choice
        else:
            raise NotImplementedError
        # print(scores, scores.size(), id)
    ...
    def _val_and_stat(self, dir, paths, val_episodes=1000):
        ...
        for val_path in paths:
            ...
            print(">> Val Score on {}: NO_AUG_Score: {} -> {}, AUG_Score: {} -> {}".format(val_path, no_aug_score_list, moe_no_aug_score, aug_score_list, moe_aug_score))
            print(">> Val Score on {}: NO_AUG_Gap: {} -> {}%, AUG_Gap: {} -> {}%".format(val_path, no_aug_gap_list, moe_no_aug_gap, aug_gap_list, moe_aug_gap))
